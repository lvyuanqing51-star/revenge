...
with st.expander("📤 录入新对局战绩（点击展开上传截图与核对）", expanded=has_pending_review):
            ...
            if st.button("🚀 开始 AI 识别（进入待复核）", type="primary", key="btn_start_ai_process"):
                ...
                for idx, (img_name, img_bytes) in enumerate(images_to_process):
                    ...
                    if img_hash in existing_hashes:
                        st.warning(f"⚠️ {img_name} 此前已录入，已跳过！")
                    ...
                    with st.spinner(f"正在识别 ({idx + 1}/{total}): {img_name}..."):
                        try:
                            ...
                            st.session_state.staging_records.append(item_entry)
                        except Exception as e:
                            st.error(f"❌ {img_name} 解析出错: {e}")
                    pbar.progress((idx + 1) / total)

                if st.session_state.staging_records:
                    st.success(f"🎉 成功识别 {len(st.session_state.staging_records)} 张截图，请在下方复核！")
                    time.sleep(0.5)
                    st.rerun()
    ...
    if st.session_state.staging_records:
        st.markdown("---")
        st.markdown("#### 🔍 识别结果人工复核与修改")
        st.info("💡 可以在右侧表格直接双击修改选手昵称与 K/D/A。确认无误后点击最下方的入库按钮。")

        for i, item in enumerate(st.session_state.staging_records):
            st.markdown(f"**对局 {i+1}：`{item['filename']}`**")
            c_preview, c_edit = st.columns([1, 2])

            with c_preview:
                st.image(item["image_bytes"], caption="原对局结算图", use_container_width=True)
                item["winning_team"] = st.selectbox(
                    f"获胜方 (对局 {i+1})",
                    options=["BLUE", "RED"],
                    index=0 if item["winning_team"] == "BLUE" else 1,
                    key=f"win_team_select_{i}"
                )

            with c_edit:
                edit_df = pd.DataFrame(item["players"])
