...
with st.sidebar:
    ...
    api_key = st.text_input("通义千问 API Key (DashScope)", value=default_key, type="password", help="以 sk- 开头的阿里云百炼密钥")
    ...
    with st.expander("🔒 管理员功能"):
        ...
        if admin_pwd == "666888":
            if current_records:
                # 永久清洗纠正按钮
                if st.button("🔄 立即强制合并同名玩家 (修复两个千秋)"):
                    if clean_database_names():
                        st.toast("已完成底层数据库清洗与合并！", icon="✅")
                    else:
                        st.toast("数据已是最新合并状态！", icon="ℹ️")
                    time.sleep(0.8)
                    st.rerun()
